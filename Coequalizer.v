...
Section Coeq_prod_commute.
  ...
  Proof.
    refine (prod_coeq_to_coeq_prod _ _ o _).
    - apply path_prod. exact pointed_f. exact pointed_f'.
    - refine (functor_prod _ idmap).
      refine (prod_coeq_to_coeq_prod _ _). exact pointed_f.
  Defined.
...
(* (equiv_prod_ind _) *)
  ...
  Definition badbadname
    ...
    forall c : Coeq f g, forall c' : Coeq f' g',
      (pullback_pc_to_cp P (prod_coeq_to_coeq_prod a0 pointed_f (c, c'))) -> P c c'.
    Proof.
    ...
    : (forall p : Coeq (functor_prod f f') (functor_prod g g'), pullback_pc_to_cp P p)
      ...
      - exact (prod_coeq_to_coeq_prod a0 pointed_f).
      - refine (prod_ind _ _).
        refine (badbadname pointed_f P).
    Defined.

...
(*   Defined. *)
  ...
  Proof.
    transitivity (ap011 BSigma_to_Z (ap (fin_to_BSigma) alpha) (ap (fin_to_BSigma) alpha)).
    { destruct alpha. reflexivity. }
    refine (_ @ concat_Vp (path_base_0 (canon_BSigma a))).
    cut (forall (x : BSigma) (p : fin_to_BSigma (canon a) = x),
            ap011 BSigma_to_Z p p = ((path_base_0 (canon_BSigma a))^) @ path_base_0 x).
    { intro H. apply H. }
  ...
  Proof.
    ...
    - intros.  hnf.
      ...
      rewrite ap_pp.
      assert (det_inv : forall (sigma : Fin a <~> Fin a),
                 determinant a sigma = determinant a (sigma^-1)).
      { intro sigma.
        transitivity (equiv_inverse (determinant a sigma)).
        { refine (_ @ ecompose_e1 _).
          apply emoveL_Ve. apply invol_SymGrp2. }
        refine ((ecompose_e1 _)^ @ _).
        apply emoveR_Ve.
        refine (_ @ det_compose _ _ _). apply inverse.
        rewrite ecompose_eV. apply det_id. }
      rewrite (det_inv (pft_inv betta)).
      refine (_ @ 
                concat2 (path_is_det _ (pft_inv alpha)) (path_is_det _ (pft_inv betta)^-1) @ _).
      + change (fin_to_BSigma (canon a)) with (canon_BSigma a).
        refine (_ @ concat_p_pp _ _ _). refine (_ @ concat_p_pp _ _ _). apply whiskerL.
        refine (_ @ concat_pp_p _ _ _). refine (_ @ concat_pp_p _ _ _).
        apply whiskerR.
        ...
        assert
          (forall (x : Finite_Types a) (p : canon a = x),
              ...
              ap011 BSigma_to_Z (path_BSigma (canon_BSigma a) (fin_to_BSigma x) (pft_inv p)) idpath).
        { intro x. destruct p. simpl.
          rewrite path_BSigma_1. reflexivity. }
        apply concat2; apply X.
      + rewrite <- path_BSigma_fix. rewrite <- path_BSigma_fix.
        assert (forall (x : Finite_Types 2) (p : canon 2 = x),
                   ap011 BSigma_to_Z (pft_to_pbs p) idpath =
    ...
    - intros. hnf. unfold ispointed_compose.
      ...
      apply moveR_pM.
      (* rewrite grp_compl_BSigma_rec_beta_path_Z. *)
      refine (grp_compl_BSigma_rec_beta_lcancel_Z
                _
                (fun A B : BSigma => BDet_uncurry (sum_BSigma A B)) _ _
                (canon_BSigma s) (canon_BSigma a) (canon_BSigma a)
                @ _).
      refine (BDet_SASB_canon_id s a a @ _). unfold BDet_SASB_canon.
      apply whiskerL. unfold BDet_sum2.
      apply (ap inverse).
      apply inverse .
      assert (forall (A B : BSigma) (p : A = B),
                 ap grpcompl_to_fin2 (ap011 BSigma_to_Z p p) =
                 ap BDet_uncurry (ap011 sum_BSigma p p)).
      { intros A B []. reflexivity. }
      refine (X _ _ (finsum_id s a) @ _).  clear X.
      unfold finsum_id. rewrite <- path_BSigma_fix.
      unfold sum_finite_types_canon.
      unfold canon_BSigma. unfold fin_to_BSigma.

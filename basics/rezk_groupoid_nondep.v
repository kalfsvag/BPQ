...
Section rezk_induction.
  ...
  Context (C : PreCategory) (H : Functor C (groupoid_category X))
          (isfullyfaithful_H : IsFullyFaithful H)
          (isessentiallysurjective_H : IsEssentiallySurjective H).

  Local Definition H_0 : C -> X.
  Proof.
    intro c. exact (H c).
  Defined.
  
  Local Definition H_1 {c1 c2 : C}
        : morphism C c1 c2 -> H_0 c1 = H_0 c2.
  Proof.
    intro g.
    exact (morphism_of H g).
  Defined.

  Instance isequiv_H1 {c1 c2 : C}: IsEquiv (@H_1 c1 c2).
  Proof.
    unfold H_1.
    unfold IsFullyFaithful in *.
    apply Morphisms.isequiv_isiso.
  Defined.    

  Local Definition H_2 {c1 c2 c3 : C} (g : morphism C c1 c2) (h : morphism C c2 c3) 
    : H_1 (h o g) = H_1 g @ H_1 h.
  Proof.
    apply (composition_of H).
  Defined.

  Definition H_idhom (c : C)
    : H_1 (c1 := c) 1 = idpath.
  Proof.
    apply (identity_of H).
  Defined.
  ...
  Lemma q_is_f (c1 c2 : C) (y : Y) (q : B (H c2) y) (h : morphism C c1 c2) :
    q.1 c1 (H_1 h) =
    (f1 h) @ (q.1 c2 idpath).
  Proof.
  ...
  Proof.
    ...
    - intro p0. destruct p0.
      ...
      + hnf. intros.
        refine (_ @ f2 _ _).
        apply (ap f1).
        apply (moveR_equiv_V (f := H_1)).
        apply inverse.
        refine (H_2 _ _ @ _).
        apply whiskerL. apply eisretr.
    - intros []. refine (_ @ f1_idhom _).
      apply (ap f1). apply moveR_equiv_V. apply inverse.
      apply H_idhom.
    - intros [q b]. destruct (q c idpath). hnf.
      apply path_sigma_hprop. simpl.
      apply path_forall. intro c1.
      apply path_forall. intro p.
      apply inverse.
      
      srefine (q_is_f c1  c (f0 c) (q; b) _ @ _).
      apply (p_is_f y (p; H)).
  Defined.

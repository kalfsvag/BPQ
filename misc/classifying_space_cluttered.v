  ...
  Definition B_ind {M : Monoid}
    ...
    - intros [[m1 m2]].
      ...
      + (*Variable runs along resp_mult (I think. . . )*)
        ...
        (* set (f := fun x : S1 => transport C (pp (m1, m2, x)) *)
        (*                                   (restriction_to_pre_B (looptofill_curried M (m1, m2, x)))). *)
        (* set (g := fun x : S1 => transport C (pp (m1, m2, base)) c0). *)

        transparent assert (pathtohub :
                              (forall x : S1,  pushl (m1, m2, x) = push (inr (m1, m2)) :> B M)).
        { intro x. exact (pp (m1, m2, x)). }
        unfold pushl in pathtohub. unfold looptofill_curried in pathtohub.
        unfold prod_curry in pathtohub. simpl in pathtohub.
        enough (restriction_to_pre_B oD (looptofill M m1 m2) =
                (fun x : S1 => transport C (pathtohub base @ (pathtohub x)^) c0)).
        transitivity (ap (fun x : S1 =>
                            transport C (pathtohub x)
                             ((restriction_to_pre_B oD looptofill M m1 m2) x)) loop).
        reflexivity.

        refine ((apD (fun f : (forall x : S1, C (push (inl (looptofill M m1 m2 x)))) =>
               ap (fun x : S1 => transport C (pathtohub x) (f x)) loop) X^)^ @ _).
        refine (transport_paths_FlFr X^ _ @ _).
        apply moveR_pM. refine (_ @ (concat_1p _)^). apply (moveR_Vp).
        refine (_ @ (concat_pV _)^).
        assert (forall x : S1,
                  transport C (pathtohub x)
                            (transport C (pathtohub base @ (pathtohub x)^) c0) =
                  transport C (pathtohub base) c0).
        { intro x.
          rewrite <- transport_pp. hott_simpl. }
        rewrite <- (ap12 loop (path_forall _ _ (fun x => (X0 x)^))).
        rewrite ap_const.
        rewrite concat_p1.
        apply concat_Vp.
        ...
        srapply @S1_ind.
        * simpl.
          apply (transport2 C (p:=idpath) (concat_pV _)^ c0).
        * refine (transport_paths_FlFr_D loop _ @ _).
          ...
          apply moveL_Mp.
          refine (concat_p1 _ @ _).
          apply inverse. hott_simpl.
          
          
          rewrite (S1_rec_beta_loop (pre_B M) (point _) ((pre_B_loop m1 @ pre_B_loop m2) @ (pre_B_loop (m1 + m2))^)).
        ...
        unfold restriction_to_pre_B.

        assert (transport
                  (fun x : S1 =>
                     restriction_to_pre_B (looptofill M m1 m2 x) =
                     transport C (pathtohub base @ (pathtohub x)^) c0) loop (transport C loop idpath) = idpath).


        refine (transport_paths_FlFr_D (f := f) (g := g) loop idpath @ _). simpl.
        apply moveR_pM. refine (concat_p1 _ @ _ @ (concat_1p _)^). apply inverse2.
        refine (_ @ (apD_const g loop)^).
        refine (_ @ (whiskerL _ (ap_const loop _))^).
        unfold g. clear g. refine (_ @ (concat_p1 _)^).
        set (h := fun (x : S1) (b : C (push (inl (looptofill M m1 m2 x)))) =>
                    transport C (pp (m1, m2, x)) b).
        change f with (composeDD h (restriction_to_pre_B oD (looptofill M m1 m2))).
        clear f.
        refine (apD_composeDD h _ loop @ _). simpl.
        apply moveR_Mp.
        ...
        (*                       (forall x : S1,  pushl (m1, m2, x) = push (inr (m1, m2)) :> B M)). *)
        (* { intro x. exact (pp (m1, m2, x)). } *)
        (* unfold pushl in pathtohub. unfold looptofill_curried in pathtohub. unfold prod_curry in pathtohub. simpl in pathtohub. *)
        (* set (p := pathtohub base). unfold looptofill in p. simpl in p. *)
        (*pathtohub base : point (B M) = push (inr (m1, m2))*)
        
        set (P := fun xc : {x : S1 & C (push (inl (looptofill M m1 m2 x)))} =>
                transport C (pp (m1, m2, xc.1)) xc.2 = transport C (pp (m1, m2, base)) c0).
        ...
        refine (concat_p1 _ @ _).
        apply (inverse2 (q := idpath)).
        set (f := (fun x : {a : S1 & C (push (inl (looptofill M m1 m2 a)))} =>
                     transport C (pp (m1, m2, x.1)) x.2)). unfold pushr in f. unfold collapseS1 in f.
        rewrite ap_section.
        unfold composeD.
        change (restriction_to_pre_B (looptofill M m1 m2 base)) with c0.
        refine (_ @ ap_1 (base; c0) f).
        apply (ap (ap f)).
        (* refine (path2_sigma _ _ _ _ _ _ _ _ _ @ _). *)


        (*         path2_sigma *)
        clear f.
        (*Not sure if this has done anything good. . .*)
        refine (path2_sigma (fun a : S1 => C (push (inl (looptofill M m1 m2 a))))
                           (base; c0) (base; c0)
                           loop idpath
                           (apD (fun x : S1 => restriction_to_pre_B (looptofill M m1 m2 x)) loop) idpath
                           _ _).
